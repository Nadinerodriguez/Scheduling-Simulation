...
public class SRTF extends Scheduler {
    ...
    private void newArrivalsReport(List<Process> pool) {
        boolean newArrivals = false;
        while ((poolIndex < processesTotal) && (systemTime == pool.get(poolIndex).getArrivalTime())) {
            enqueue(pool.get(poolIndex));
            newArrivals = true;
            poolIndex++;
        }
        if (currentProc != null && newArrivals) {
            preemptionReport(getHead(), currentProc);
        }
    }

    private void preemptionReport(Process p, Process cp) {
        if (p.getCPUBurstRemaining() < cp.getCPUBurstRemaining()) {
            currentProc = super.removeHead();
            Process c;
            ...
            if (!(super.rqIsEmpty())) {
                for (int i = 0; i < super.rqSize(); i++) {
                    c = super.getProcess(i);
                    if (cp.getCPUBurstRemaining() <= c.getCPUBurstRemaining()) {
                        super.addProcess(i, cp);
                        return;
                    }
                }
            }
    ...
    private void enqueue(Process p) {
        ...
        if (!(super.rqIsEmpty())) {
            for (int i = 0; i < super.rqSize(); i++) {
                c = super.getProcess(i);
                if (p.getCPUBurstRemaining() < c.getCPUBurstRemaining()) {
                    super.addProcess(i, p);
                    return;
                }
            }
        }
